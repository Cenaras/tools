	...
	for {
		...
		*/
		...
		for _, wc := range a.waveConstraints {
			...
			var pnew nodeset
			pnew.Difference(&a.nodes[id].solve.pts, &wc.cache)
...
func (a *analysis) processNewConstraints() {
	...
	for _, c := range constraints {
		...
		default:
			...
			a.waveConstraints = append(a.waveConstraints, &waveConstraint{constraint: c, cache: newNodeSet()})
		}
		/*
